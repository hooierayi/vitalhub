...
| 组件 | 位置 | 约束 |
|---|---|---|
| 路径与参数常量 | `core/common/.../navigation/Routes.kt` | 新增跨模块入口先定义稳定路径与参数名 |
| 跳转封装 | `core/common/.../navigation/Navigator.kt` | 跨模块调用复用此处封装，避免在 feature 间直接引用类 |
...
当前用户资料编辑路由为 `/user/edit`，由 `:feature:user` 的 `UserInfoEditFragment` 注册；首页通过 `Navigator.editUserInfo` 进入该子流程。首页四步采集进度由 `CollectionProgressProvider` 的 `/collection/progress/service` 服务读取和更新，不作为路由参数传递。
...
- 当前 `CollectionProgressProvider` 以 `/collection/progress/service` 注册，由首页和流程模块通过 `ARouter.navigation(CollectionProgressProvider::class.java)` 获取；设备连接和采集的硬件瞬态在返回首页时回退为重新连接设备。

## 3. UI 壳约束

- 顶级底栏目前包含采集、记录、报告、我的；后 3 项目前由 app 内占位页面承载。
- 顶级 Tab 不进入业务返回栈；子流程默认入栈并隐藏底栏。
- 标题栏返回与系统返回均走 Activity 的返回栈。
