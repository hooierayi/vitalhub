...
- 当前 `RecordProvider` 以 `/record/service` 注册，由采集模块保存完成记录、app 记录页观察全部记录；点击记录卡片时只将唯一 `recordId` 和 `FROM_RECORD` 来源交给分析页，分析页按 `recordId` 通过 Provider 恢复完整记录、`analysisId` 和错误恢复所需的 `sessionId`，Room 实现不暴露到契约或 app。采集完成进入分析页时使用 `FROM_COLLECTION` 来源；分析页来源与 `flowEntryMode` 返回策略相互独立。
- 进入 `/collection/flow` 前由应用层用户资料拦截器查询 `UserInfoProvider`；无完整资料时中断原路由并重定向 `/user/edit`。该拦截器先于蓝牙权限守卫执行，避免在用户资料未就绪时提前申请设备权限。

## 3. UI 壳约束

- 顶级底栏目前包含采集、记录、报告、我的，顶部以细分界线区分页面内容；记录页通过 `RecordProvider` 展示本地完成记录，点击记录卡片以直达模式进入对应分析页。记录流中的用户查询、日期与时长格式化在后台线程完成，用户指纹查询在页面生命周期内去重缓存，列表仅消费稳定 UI 模型，避免切页和滚动时阻塞主线程；报告和我的仍由 app 内占位页面承载。顶级页面只预留底栏占用空间，不重复叠加系统导航栏安全区。
