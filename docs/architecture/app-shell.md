...
- 标题栏返回与系统返回共用 Activity 返回栈策略：片段采集、连续记录在 `CollectionFlowActivity` 内回到实时预览；倒计时完成进入上传与分析页时，等待 `AnalysisActivity` 到达后销毁 `CollectionFlowActivity`，上传与分析页返回时直接恢复首页；问卷返回通过结束 `QuestionnaireActivity` 直接恢复其下的页面，其他子流程按所属 Activity 的 Fragment 返回栈逐级回退。
