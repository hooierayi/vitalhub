...
- 默认产物按 `artifacts/apk/<debug|release>/<versionName>/` 分类；每个构建类型的每个版本目录只保留最新一份 APK 及其同名 `.txt` 构建信息。APK 文件名依次包含应用名称、版本名称、versionCode、Git 短提交号、工作区状态、连续数字构建时间和 debug/release 类型。
