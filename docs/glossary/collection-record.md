...
- 每条记录以 App 本地 `id` 唯一标识，并保存 `sessionId`、采集类型、记录时间、时长、本地文件路径、`userFingerprint` 和 `deviceAddress`。完成上传后还保存 `analysisId`、分析状态、Markdown 结果或错误信息，使页面可按 `sessionId` 恢复。片段记录编号使用 `CLIP-` 前缀，后接 UUID 去连字符后的前 12 位十六进制字符；历史 `REC-` 编号保持不变。
- 只有片段采集完成才写入；实时预览、暂停、中途退出和设备内部连续写卡均不构成采集上传记录。

## 2. 所属边界

- `:provider:record` 定义 `CollectionRecord` 与 `RecordProvider` 契约。
- `:feature:collection` 使用 Room 保存记录与其分析摘要，并以 `/record/service` 注册 ARouter Provider；`:feature:analysis` 只经该 Provider 查询记录和回写分析状态。
