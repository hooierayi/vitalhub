...
- 流程：首页无资料时展示引导卡；所有进入采集 Activity 的路由都由应用层拦截器检查资料，无完整资料时重定向到填写页；保存后回到首页展示真实摘要并恢复正常采集跳转。后续进入编辑页会回填资料并显示“修改用户信息”。详情见 `feature/user/docs/biz/features/user-profile.md`。
