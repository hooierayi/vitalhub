...
1. 首页通过 ARouter 获取 `UserInfoProvider`。未读取到完整资料时，展示“用户信息未填写”引导卡；卡片和底部“填写用户信息”均进入 `/user/edit`。进入采集 Activity 的路由由应用层拦截器统一查询 `UserInfoProvider`，无完整资料时中断采集路由并重定向到 `/user/edit`。
