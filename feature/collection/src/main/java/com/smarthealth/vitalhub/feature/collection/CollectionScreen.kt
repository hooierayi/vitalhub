...
fun CollectionScreen(
    ...
    when (state.mode) {
        ...
        else -> PreviewPage(state, device, isDeviceConnected, onStartClip, onStartContinuous)
    }
}
...
private fun PreviewPage(
    ...
    FlowPage(scrollable = false) {
        ConnectedBanner(device, isDeviceConnected); Waveform("ECG", "增益：10 mm/mV", height = 95.dp); Waveform("RESP", "阻抗", true, 95.dp); Spacer(Modifier.height(14.dp))
...
private fun ConnectedBanner(device: BluetoothKitDevice?, isConnected: Boolean) {
    ...
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .height(54.dp)
            .background(Color(0xFFF0F8F5), RoundedCornerShape(10.dp))
