...
- 服务端当前只返回 Markdown 结果，因此页面移除 Mock 指标卡，并使用 Markwon 在原报告卡片内渲染报告内容；空结果仍显示占位文案。渲染器启用 CommonMark、GFM 表格/删除线/任务列表、HTML、脚注、Markdown 图片（包含常规位图、GIF、SVG）、PDF 链接、块级与行内 LaTeX、代码语法高亮、URL/邮箱/电话自动链接和软换行；Markdown 渲染、PDF 查看与 Prism 配置统一归档在 `feature/analysis/.../markdown/` 目录。Markdown 交互元素统一使用 GitHub 风格蓝色 `#0969DA`，不跟随业务主题色；表格边框、代码背景和引用线使用中性灰。报告内 PNG、SVG 与 GIF 根据内容区可用宽度和原始宽高比动态计算尺寸，优先填满内容宽度；极端宽高比使用受限展示框并保持图片比例。SVG 加载后裁掉透明画布边缘；GIF 使用兼容性解码器，按首帧有效内容范围计算布局并保留运动缓冲边距，同时维持 Drawable 回调链以持续播放动画。点击图片在当前页面以模态 Dialog 放大预览；预览复用正文图片加载器及缓存，未命中缓存时显示加载状态，内容卡片保留 24dp 左右边距。行内 LaTeX 同时兼容常见的单 `$...$` 与 Markwon 原生的双 `$$...$$` 定界符；邮箱使用独立的 ASCII 地址匹配；自动电话只识别带 `+` 且包含 10 至 15 位数字的明确国际号码，避免把指标范围识别成拨号链接。顶层 GFM 表格独立渲染并支持横向滑动，表格内链接保留点击能力；`[^label]` 脚注显示为较大的上标括号链接，并支持正文、定义双向跳转；指向 HTTP(S) `.pdf` 的 Markdown 链接在应用内下载并分页展示，单文件上限 25 MB，PDF 页使用白色状态栏与标题栏并以居中缩放动画打开。代码高亮生成器包含 Prism4j 提供的全部语言定义。
- Debug 构建在分析网络客户端中注册 `AnalysisMockInterceptor`，但默认不启用；通过 DoKit 的“分析结果 Mock”开关持久化控制。开启后，上传接口立即返回已受理，结果接口立即返回覆盖全部已启用格式的 Markdown 样例，且不要求配置分析 API Key；关闭时与 Release 一样调用真实分析服务。
- Markdown 预处理、CommonMark 解析和 Span 构建在后台线程完成，并按原文缓存渲染结果；Compose 重组只复用已解析的 `Spanned`，不重复执行整段 `setMarkdown`。图片后处理明确排除 LaTeX Span，避免行内公式被误包装；SVG/GIF 透明边界通过批量像素读取扫描，减少主线程 JNI 调用和滚动卡顿。
